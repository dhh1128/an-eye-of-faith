...
STD_LINK_PATTERN = re.compile(r'https?://aneyeoffaith.wordpress.com/(\d{4}/\d{2}/\d{2})/(.+)')
UPLOAD_LINK_PATTERN = re.compile(r'https?://aneyeoffaith.wordpress.com/wp-content/uploads/(\d{4}/\d{2}(?:/\d{2})?)/(.+)')
DRAFT_LINK_PATTERN = re.compile(r'https?://aneyeoffaith.wordpress.com/\?p=(\d+)')
...
class Post:
    ...
    def save(self):
        ...
        try:
            ...
            print(f"Error saving post {self.url} to {fname}: {e}")
